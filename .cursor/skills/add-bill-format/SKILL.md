...
4. 仅当账单文件为文本类型（方便git跟踪）时，在 `tests/test_sources/` 新增对应测试（例如 `test_bank_xxx.py`）。例如CSV格式需要新增测试用例，XLSX等二进制格式则不需要。
5. 测试至少覆盖以下场景：
   - 基本解析成功（行数与关键字段正确）。
...
- 新增 fixture、解析器、registry 注册已落地。
- 如适用，新增测试用例并通过，且不破坏已有测试。
- 代码与命名风格符合项目规则。
